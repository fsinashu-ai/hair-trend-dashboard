...
- `/seo/integrated`: Search Console・GA4・広告のページ統合分析
- `/ads`: 広告管理メモ
- `/ads/reports`: 広告月次レポート
- `/ads/creatives`: Gemini広告案生成

SEOと広告の数値は、Google Search Console、GA4、Google広告から取得・取り込みした内容を整理するためのMVPです。Search ConsoleはCSV取り込み、GA4はCSVまたは公式Data API、Google広告は読み取り専用の公式APIまたはCSVに対応しています。広告の自動運用やWordPress自動投稿は実装していません。

AI改善提案はGeminiを利用します。`GEMINI_API_KEY`が未設定、無効、利用制限中の場合も画面を止めずモック文を表示します。

### データの見方

SEO・広告の画面には、データの出所、対象期間、最終保存・更新時刻、分析に含むもの、確認が必要なことを表示します。数値を見る前に、各画面で対象データを確認してください。
...
| `/ads/imports` | 広告費、表示、クリック、CTR、CV、CPAの集計 | Google広告APIの保存結果、または広告CSV | 選択した1つの取り込みデータを集計します。Meta・InstagramはCSV取り込みが必要です。 |
| ホームの集客ダッシュボード | SEO・広告・ブログ・LINE導線の概要 | 各機能の最新保存データ | カードごとにデータ元を表示します。未取り込み項目はサンプルで補う場合があります。 |

SEO・広告の上部ナビは、次の順番で使います。

1. `計画・管理`で、対策キーワード・改善ページ・広告方針・タスクを整理します。
2. `データを取り込む`で、Search Console CSV、GA4、Google広告API、広告CSVを保存します。
3. `実績を分析する`で、対象期間とデータ元を確認してから改善候補を見ます。
4. `公開前確認`で、Geminiが作った記事や広告文を確認します。

### Phase 3: SEO実データ優先化

`/seo` は、サーバー側でSupabaseに保存された最新のSearch ConsoleとGA4取り込みを読み込みます。Search Consoleのクリック・表示・CTR・平均掲載順位、GA4のユーザー・セッション・表示回数・LINEクリック・予約クリックを概要で確認できます。

まだ取り込みがない項目は参考データに戻ります。画面上に「実績」「参考データ」「Gemini未実行」などの状態を表示するため、サンプル値と分析済みの実績を区別できます。`SUPABASE_SERVICE_ROLE_KEY`が未設定の場合は、既存の各CSV画面やこの端末の保存データを使ってください。

### Phase 4: 広告実データ優先化

`/ads` は、Supabaseに保存された最新のGoogle広告API取得または広告CSVを優先して表示します。Supabaseが未設定の場合でも、この端末に保存された広告CSVがあれば表示できます。

表示する実績は広告費、表示回数、クリック、CTR、コンバージョン、CPAです。広告方針や予算メモは別に管理し、配信結果と混ざらないようにしています。広告の詳細な期間比較、キャンペーン別集計、改善候補は `/ads/imports` で確認します。

### Phase 5: Geminiの役割

Geminiは、アプリ側で計算した集計値と改善候補をもとに、総評・改善案・記事案・広告案・タスク案を作成します。CTR、順位、CPAなどの数値計算やしきい値判定はアプリ側で行い、GeminiにCSVの全行は送信しません。

広告の出稿、停止、予算変更、SEOの公開、外部サービスの設定変更は行いません。生成内容は必ず人が事実、表現、予算、公開可否を確認してから使ってください。

### Phase 6: ページ統合分析

`/seo/integrated` では、Search Console、GA4、広告の最新取り込みをページパス単位で照合します。URLのドメイン、クエリ文字列、末尾スラッシュを整えてから同じページとしてまとめるため、検索流入・サイト内行動・広告結果を横並びで確認できます。

データがない列は「未取得」と表示し、推測で補完しません。優先度はアプリ側のしきい値で判定し、Geminiはページごとの改善案を作成するだけです。
...
GA4でLINEクリックなどを正しく計測できていなかった期間を確認するため、定例メールから整理した2022年12月〜2026年6月の月次数値を補完コンテキストとして収録しています。`/seo/ga4`では、選択したGA4期間と同じ月のSearch Consoleクリック、LINEリンク、友だち追加、広告クリック・費用などを別枠で確認できます。

- 補完値はGA4・広告・Search Consoleの実績へ自動加算しません。
- 空欄は未記載・未確定であり、0とは区別しています。
- サイト人数/PVは月によって指標定義が異なるため、指標種別と一緒に確認します。
- 広告CV、LINEリンク、友だち追加、相談、予約は別の指標です。
- Geminiには全メールではなく、対象期間の月次要約と注意事項だけを渡します。

正規化済みデータは`src/data/emailMonthlyMetrics.json`、集計ルールは`src/lib/emailMetrics.ts`で管理しています。元Excelを更新した場合は、数字と空欄の区別を確認してから正規化データも更新してください。
...
## Final Dashboard: 毎朝見る画面

ホーム画面（`/`）では、次の情報を1画面で確認できます。

- 今月のSEO状況
- 広告費、クリック、CTR、コンバージョン、CPA
- ブログの下書き・公開状況
- LINEクリックと予約クリック
- Search Console、GA4、広告をページ単位で照合した統合分析
- 未完了タスク、今日やること、今月やること
- Geminiの総評と、各データの取得元・対象期間

使い方:

1. まずホーム画面の各カードで、数字と「データ元」を確認します。
2. 「ページ統合分析」に高優先のページがあれば、`/seo/integrated` を開きます。
3. SEO、広告、LINE導線の改善を行う前に、対象期間が同じか確認します。
4. Geminiの提案は下書きとして読み、人が事実・表現・公開可否を確認してから使います。

Search Console、GA4、広告のいずれかが未取り込みの場合は、画面に「未取得」またはサンプルを含む表示が出ます。サンプルを実績と取り違えないよう、各カードのデータ元を確認してください。
